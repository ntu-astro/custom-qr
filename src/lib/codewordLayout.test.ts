...
describe('getEccLayoutForH', () => {
  it('matches the QR spec for version 3 (used by https://ntuastro.com)', () => {
    const m = buildMatrix('https://ntuastro.com');
    expect(m.size).toBe(29); // version 3 = 4*3 + 17 = 29
    const layout = getEccLayoutForH(m.size);
    expect(layout.version).toBe(3);
    expect(layout.totalCodewords).toBe(70);
    expect(layout.dataTotalCodewords).toBe(26); // 70 - 44
    expect(layout.ecTotalBlocks).toBe(2);
    expect(layout.ecCount).toBe(22); // 44 ECC / 2 blocks
    // 26 data codewords / 2 blocks → 13 each, all in group 1.
    expect(layout.dataCodewordsInGroup1).toBe(13);
    expect(layout.dataCodewordsInGroup2).toBe(14);
    expect(layout.blocksInGroup1).toBe(2);
    expect(layout.blocksInGroup2).toBe(0);
  });
